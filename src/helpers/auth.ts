...
export const secureUserData = (user: Record<string, any>, req: Request | null=null, additionalProps: Record<string, any>={}, currentAccessToken: string | null = null)=>{
    ...
    try {
        ...
        return {data: {...user, ...nullables, token: accessToken, ...additionalProps}}
    } catch (error) {
        console.log(error);
        return {data: {...user, ...nullables, ...additionalProps}}
    }
...
export const generateVerificationCode = async (database: Knex, userId: string | null =null, email: string | null =null, currentAlertLevel=-1, toggleAlertLevel=false)=>{
